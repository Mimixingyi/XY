...
$(window).ready(function() {
	...
	$(window).scroll(function(e) {
		...
		//判断滚动条是否超过200px
		if (sTop >= 200) {
			topcontrlo.fadeIn(300);
		}else {
			topcontrlo.fadeOut(300);
		}
		...
		if (sTop >= navPos) {
			if (!(nav.hasClass('fix'))) {
				nav.addClass('fix');
				$('.banner').css('margin-bottom',navHeight);//在banner下面空出等高的占位
			}
		}else {
			if (nav.hasClass('fix')){
				nav.removeClass('fix');
		...
		if (sTop >= choicePos.start && sTop < choicePos.end) {
			...
			if ( $('.choice').hasClass('active') ) {
				$('.choice').removeClass('active');//失去高亮
			}
		}
	...
	//回到顶部按钮单击事件
	topcontrlo.click(function() {
		//滚动条过渡到顶部
		$('html,body').animate({scrollTop:0},1000);//元素相对于网页原点的位置
	});
